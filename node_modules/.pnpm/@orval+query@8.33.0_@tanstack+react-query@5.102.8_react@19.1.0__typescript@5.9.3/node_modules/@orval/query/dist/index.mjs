import { GetterPropType, OutputClient, OutputHttpClient, Verbs, camel, compareVersions, createLogger, emitResponseValidation, escapeRouteForSingleQuotes, generateAxiosUrl, generateFormDataAndUrlEncodedFunction, generateMutator, generateMutatorConfig, generateMutatorRequestOptions, generateOptions, generateResponseDateDeserializer, generateVerbImports, getAngularFilteredParamsCallExpression, getAngularFilteredParamsHelperBody, getFullRoute, getRoute, getRouteAsArray, getSchemaOutputTypeRef, getSchemaValueRef, getSuccessResponseType, hasSchemaImport, isObject, isOperationInTagBucket, isPrimitiveResponseType, isString, isSyntheticDefaultImportsAllow, jsDoc, jsStringLiteralEscape, makeRouteSafe, mergeDeep, pascal, rewriteImportsForResponseValidation, stringify, toObjectString } from "@orval/core";
...
*/
const getOperationMetaLiteral = (operationId, operationName) => `operationId: '${jsStringLiteralEscape(operationId)}', operationName: '${jsStringLiteralEscape(operationName)}'`;
//#endregion
...
const generateAxiosRequestFunction = ({ headers, queryParams, operationName, urlHelperName, response, mutator, body, props: _props, verb, formData, formUrlEncoded, override, paramsSerializer, params }, { route: _route, context }, adapter) => {
  ...
  ${dateDeserializerImplementation}`;
	...
	const urlProps = props.filter((prop) => prop.type === GetterPropType.PARAM || prop.type === GetterPropType.NAMED_PATH_PARAMS || prop.type === GetterPropType.QUERY_PARAM);
...
const logger = createLogger("@orval/query");
//#endregion
...
const generateQueryOptions = ({ params, options, type, adapter, useSkipToken }) => {
	...
	if (useSkipToken) return `${queryConfig} ...queryOptions`;
...
const QUERY_CLIENT_TYPES = /* @__PURE__ */ new Set([
	"react-query",
	"vue-query",
	"svelte-query",
	"angular-query",
...
*/
const asQueryClientType = (outputClient) => isString(outputClient) && QUERY_CLIENT_TYPES.has(outputClient) ? outputClient : void 0;
/**
* Clients already warned about, so an undetectable version is reported once per
* run rather than once per operation.
*/
const undetectedVersionWarnings = /* @__PURE__ */ new Set();
/**
...
*/
const warnOnUndetectedQueryVersion = (outputClient, packageJson, queryVersion) => {
	const clientType = asQueryClientType(outputClient);
	if (clientType === void 0 || queryVersion !== void 0 || clientType === "angular-query" || getPackageByQueryClient(packageJson, clientType) || undetectedVersionWarnings.has(clientType)) return;
	undetectedVersionWarnings.add(clientType);
	logger.warn(`Could not determine the installed @tanstack/${clientType} version, so hooks are generated for v4.\nIf the project is on v5, set \`override.query.version: 5\` (or point \`output.packageJson\` at the package.json that declares the dependency) — otherwise the generated option types will not accept a partial \`query\` object.`);
};
/**
...
*/
const toPrefixLiteral = (prefix) => prefix.includes("${") ? `\`${prefix}\`` : `'${escapeRouteForSingleQuotes(prefix)}'`;
/**
...
${hasInvalidation ? adapter.generateMutationOnSuccess({
        ...
        ${mutationOptionsMutator ? `const customOptions = ${mutationOptionsMutator.name}({...mutationOptions, mutationFn}${mutationOptionsMutator.hasSecondArg ? `, { url: \`${getMutationOptionsUrl(route, getMutationOptionsPathParamNames(props), pathRoute)}\` }` : ""}${mutationOptionsMutator.hasThirdArg ? `, { ${getOperationMetaLiteral(operationId, operationName)} }` : ""});` : ""}
...
*/
const getSuppressedInfiniteQueryWarning = ({ operationName, infiniteHookAllowed, operationUseInfinite, operationUseSuspenseInfiniteQuery, configuredInfiniteQueryParam, queryParams }) => {
	if (infiniteHookAllowed) return void 0;
	const requested = [operationUseInfinite === true ? "useInfinite" : void 0, operationUseSuspenseInfiniteQuery === true ? "useSuspenseInfiniteQuery" : void 0].filter((flag) => !!flag);
	if (requested.length === 0) return void 0;
	const candidates = (Array.isArray(configuredInfiniteQueryParam) ? configuredInfiniteQueryParam : [configuredInfiniteQueryParam]).filter((name) => !!name);
	const declared = queryParams?.paramNames ?? [];
	const declaredText = declared.length > 0 ? `only declares ${quoteList(declared)}` : "declares no query parameters";
	return `'${operationName}' sets ${quoteList(requested)}, but its useInfiniteQueryParam (${quoteList(candidates)}) is not one of its query parameters — the operation ${declaredText}, so no infinite hook was generated. orval can only page through a URL query parameter; a page param that lives in the request body is not supported yet (#4025). Either declare it as a query parameter, or drop the infinite options for this operation.`;
};
...
*/
const getQueryFnProperty = ({ useSkipToken, params, type }) => {
	if (!useSkipToken || params.length === 0 || isSuspenseQuery(type)) return "queryFn";
	return `queryFn: ${params.map(({ name }) => `${name} === null || ${name} === undefined`).join(" || ")} ? skipToken : queryFn`;
};
/**
...
*/
const resolveUseSkipToken = (useSkipToken, adapter) => !!useSkipToken && adapter.outputClient === OutputClient.REACT_QUERY && adapter.hasQueryV5;
/**
...
const generateQueryImplementation = ({ queryOption: { name, typeName: optionTypeName, queryParam, options, type, queryKeyFnName }, operationId, operationName, typeName, queryProperties, queryKeyProperties, queryParams, params, props, body, mutator, queryOptionsMutator, queryKeyMutator, isRequestOptions, response, httpClient, isExactOptionalPropertyTypes, hasSignal, useRuntimeFetcher, forceSuccessResponse, route, doc, deprecated, usePrefetch, useQuery, useInfinite, useInvalidate, useSetQueryData, useGetQueryData, useSkipToken, adapter }) => {
	...
	const queryFnProperty = getQueryFnProperty({
		useSkipToken,
		params,
		type
	});
...
${hookOptions}
  ...
  ${mutator?.isHook ? `const ${operationName} =  use${pascal(operationName)}Hook();` : ""}
    ...
    const queryFn: QueryFunction<Awaited<ReturnType<${mutator?.isHook ? `ReturnType<typeof use${pascal(operationName)}Hook>` : `typeof ${operationName}`}>>${hasQueryV5 && hasInfiniteQueryParam ? `, QueryKey, ${infiniteQueryParamType}` : hasQueryV5 && isInfiniteQuery(type) && adapter.shouldCastQueryOptions?.() === false ? `, QueryKey, unknown` : ""}> = (${queryFnArguments}) => ${operationName}(${httpFunctionProps}${httpFunctionProps ? ", " : ""}${queryOptions});
      ...
      ${queryOptionsMutator ? `const customOptions = ${queryOptionsMutator.name}({ queryKey, ${queryFnProperty}, ${queryOptionsImp}}${queryOptionsMutator.hasSecondArg ? `, { ${queryProperties} }` : ""}${queryOptionsMutator.hasThirdArg ? `, { url: \`${route}\`, ${getOperationMetaLiteral(operationId, operationName)} }` : ""});` : ""}

   return  ${queryOptionsMutator ? "customOptions" : `{ queryKey, ${queryFnProperty}, ${queryOptionsImp}}`}${adapter.shouldCastQueryOptions?.() === false ? "" : ` as ${queryOptionFnReturnType} ${adapter.shouldAnnotateQueryKey() ? `& { queryKey: ${hasQueryV5 ? `DataTag<QueryKey, TData${hasQueryV5WithDataTagError ? ", TError" : ""}>` : "QueryKey"} }` : ""}`}
}`;
	const operationPrefix = adapter.hookPrefix;
...
export function ${queryHookName}<TData = ${TData}, TError = ${errorType}>(\n ${queryPropDefinitions} ${queryArguments} ${optionalQueryClientArgument}\n  ): ${returnType}`;
	...
	const applyQueryOptionsMutator = (baseExpr) => queryOptionsMutator && !queryOptionsMutator.isHook ? `${queryOptionsMutator.name}({ queryKey: ${baseExpr} }${queryOptionsMutator.hasSecondArg ? `, { ${queryProperties} }` : ""}${queryOptionsMutator.hasThirdArg ? `, { url: \`${route}\`, ${getOperationMetaLiteral(operationId, operationName)} }` : ""}).queryKey` : baseExpr;
	const hasHookMutator = !!queryOptionsMutator?.isHook;
	if (hasHookMutator && (useSetQueryData || useGetQueryData)) logger.warn(`'${name}' has a hook-based queryOptions mutator, so the requested set/get-query-data helpers were skipped to avoid a cache-key mismatch with the query hook.`);
...
const generateQueryHook = async (verbOptions, options, outputClient, adapter) => {
	...
	const suppressedInfiniteWarning = getSuppressedInfiniteQueryWarning({
		...
		operationUseSuspenseInfiniteQuery: operationQueryOptions?.useSuspenseInfiniteQuery,
		configuredInfiniteQueryParam,
		queryParams
	});
	if (suppressedInfiniteWarning) logger.warn(suppressedInfiniteWarning);
	...
	const useSkipToken = resolveUseSkipToken(override.query.useSkipToken, adapter);
	...
	if (conflictWarning) logger.warn(conflictWarning);
	if (isQuery) {
...
const generateQuery = async (verbOptions, options, outputClient) => {
	...
	const hasValidatableResponse = isZodOutput && !isPrimitiveResponseType(responseType) && hasSchemaImport(verbOptions.response.imports, responseType);
	const shouldUseRuntimeValidation = verbOptions.override.query.runtimeValidation?.enabled && !verbOptions.mutator && hasValidatableResponse;
	const shouldImportSchemaValue = !!verbOptions.mutator && options.context.output.httpClient === OutputHttpClient.FETCH && options.context.output.override.includeZodSchemaInArguments && verbOptions.override.fetch.runtimeValidation.enabled && hasValidatableResponse && !verbOptions.response.contentTypes.some((contentType) => contentType === "application/nd-json" || contentType === "application/x-ndjson");
	const normalizedVerbOptions = shouldUseRuntimeValidation || shouldImportSchemaValue ? {
		...verbOptions,
		response: {
			...verbOptions.response,
			imports: rewriteImportsForResponseValidation(verbOptions.response.imports, responseType, { includeOutputType: shouldUseRuntimeValidation && adapter.isAngularHttp })
		}
	} : verbOptions;
