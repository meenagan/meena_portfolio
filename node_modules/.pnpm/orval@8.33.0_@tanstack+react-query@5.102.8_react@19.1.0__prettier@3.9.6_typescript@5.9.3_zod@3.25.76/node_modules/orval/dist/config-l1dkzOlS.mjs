...
import { DefaultTag, FormDataArrayHandling, GetterPropType, NamingConvention, OutputClient, OutputHttpClient, OutputMockType, OutputMode, PropertySortOrder, RefComponentSuffix, SupportedFormatter, asyncReduce, bindReporter, buildDynamicScope, buildSchemaTagMap, collectReferencedComponents, compareNatural, conventionName, createLogger, createSchemaOutputPlanForOutput, createSuccessMessage, dynamicImport, escapePath, filterPathsBySchemas, fixCrossDirectoryImports, fixRegularSchemaImports, generateComponentDefinition, generateDependencyImports, generateMutator, generateParameterDefinition, generateSchemasDefinition, generateVerbsOptions, getBaseUrlRuntimeImports, getConfiguredMockDirectories, getFileInfo, getFullRoute, getImportExtension, getMockFileExtensionByTypeName, getOperationUrlHelperNames, getRefInfo, getRoute, isBoolean, isComponentRef, isFunction, isNullish, isObject, isReference, isString, isUrl, jsDoc, kebab, normalizeRuntimeValidation, operationZodOverrideKeys, pascal, removeFilesAndEmptyFolders, resolveInstalledVersions, resolveRef, resolveValue, splitSchemasByType, stripFileExtension, upath, withGeneratedFileTransform, writeGeneratedFile, writeRoutedSchemas, writeSchemas, writeSchemasTagsSplit, writeSingleMode, writeSplitMode, writeSplitTagsMode, writeTagsMode, writeTagsOperationsMode, writeTagsOperationsSplitMode } from "@orval/core";
...
import zod, { assertZodTarget, dereference, generateFormDataZodSchema, generateZodValidationSchemaDefinition, getZodImportSource, parseZodValidationSchemaDefinition, renderZodExport, resolveIsZodV4 } from "@orval/zod";
...
const generateClientFooter = ({ outputClient, operationNames, operations, hasMutator, hasAwaitedType, titles, output }) => {
	...
	try {
		if (isFunction(outputClient)) {
			...
			logger.warn("Passing an array of strings for operations names to the footer function is deprecated and will be removed in a future major release. Please pass them in an object instead: { operationNames: string[] }.");
...
const generateExtraFiles = (outputClient = DEFAULT_CLIENT, verbsOptions, output, context, schemaTagMap, schemaOutputPlan) => {
	const { extraFiles: generateExtraFiles } = getGeneratorClient(outputClient, output);
	if (!generateExtraFiles) return Promise.resolve([]);
	return generateExtraFiles(verbsOptions, output, context, schemaTagMap, schemaOutputPlan);
};
//#endregion
//#region src/api.ts
async function getApiBuilder({ input, output, context, componentSchemas }) {
	const pathEntries = [];
	for (const [pathRoute, verbs] of Object.entries(context.spec.paths ?? {})) {
		...
		if (output.override.useDeprecatedOperations === false) verbsOptions = verbsOptions.filter((verb) => !verb.deprecated);
		pathEntries.push({
			pathRoute,
			route,
			resolvedVerbs,
	...
	const allOperationNames = pathEntries.flatMap(({ verbsOptions }) => verbsOptions.map(({ operationName }) => operationName));
	const helperOptions = pathEntries.flatMap(({ verbsOptions }) => verbsOptions.filter(({ mutator }) => !mutator));
	const helperNames = getOperationUrlHelperNames(allOperationNames, helperOptions.map(({ operationName }) => operationName));
	const helperNamesByOption = new Map(helperOptions.map((verbOption, index) => [verbOption, helperNames[index]]));
	const api = await asyncReduce(pathEntries, async (acc, { pathRoute, route, resolvedVerbs, verbsOptions }) => {
		const resolvedVerbOptions = verbsOptions.map((verbOption) => {
			const urlHelperName = helperNamesByOption.get(verbOption);
			return urlHelperName ? {
				...verbOption,
				urlHelperName
			} : verbOption;
		});
		const schemas = [];
		for (const { queryParams, headers, body, response, props } of resolvedVerbOptions) {
		...
		const pathOperations = await generateOperations(output.client, resolvedVerbOptions, {
			route: fullRoute,
			pathRoute,
		...
		for (const verbOption of resolvedVerbOptions) acc.verbOptions[verbOption.operationId] = verbOption;
	...
	const schemaTagMap = isObject(output.schemas) && output.schemas.splitByTags ? buildSchemaTagMap(Object.values(api.operations).map((operation) => ({
		imports: operation.imports,
		tags: operation.tags
	})), [...componentSchemas, ...api.schemas]) : void 0;
	const schemaOutputPlan = createSchemaOutputPlanForOutput([...componentSchemas, ...api.schemas], output, schemaTagMap);
	const extraFiles = await generateExtraFiles(output.client, api.verbOptions, output, context, schemaTagMap, schemaOutputPlan);
	return {
...
function filterSpecComponents(spec, input) {
	...
	if (filters?.schemas) return filterPathsBySchemas(spec, filters.schemas, filters.mode);
...
function getApiSchemas({ input, output, target, workspace, spec }) {
	...
	const schemaDefinition = generateSchemasDefinition(spec.components?.schemas, context, output.override.components.schemas.suffix, input.filters, output.override.components.schemas.prefix);
	const responseDefinition = generateComponentDefinition(spec.components?.responses, context, output.override.components.responses.suffix, output.override.components.responses.prefix);
	const swaggerResponseDefinition = generateComponentDefinition("responses" in spec ? spec.responses : void 0, context, "");
	const bodyDefinition = generateComponentDefinition(spec.components?.requestBodies, context, output.override.components.requestBodies.suffix, output.override.components.requestBodies.prefix);
	const parameters = generateParameterDefinition(spec.components?.parameters, context, output.override.components.parameters.suffix, output.override.components.parameters.prefix);
	return [
		...schemaDefinition,
		...responseDefinition,
...
const loadPackageJson = async (packageJson, workspace = process.cwd()) => {
	...
	if (fs$2.existsSync(normalizedPath)) {
		let pkg;
		try {
			pkg = JSON5.parse(await fs$2.readFile(normalizedPath, "utf8"));
		} catch (error) {
			throw new Error(`Oups... 🍺. Path: ${normalizedPath} => ${String(error)}`, { cause: error });
		}
...
const resolveAndAttachVersions = (pkg, workspace, cacheKey) => {
	...
	if (Object.keys(resolved).length > 0) {
		...
		for (const [name, version] of Object.entries(resolved)) logger.verbose(styleText("dim", `Detected ${styleText("white", name)} v${styleText("white", version)}`));
	}
	return pkg;
};
...
const maybeReplaceCatalog = async (pkg, workspace) => {
	...
	if (!catalogData) {
		logger.warn("package.json contains catalog: references, but no catalog source was found (checked: pnpm-workspace.yaml, package.json, .yarnrc.yml).");
		return pkg;
	}
...
const performSubstitution = (dependencies, catalogData) => {
	...
	for (const [packageName, version] of Object.entries(dependencies)) if (version === "catalog:" || version === "catalog:default") {
		if (!catalogData.catalog) {
			logger.warn(`catalog: substitution for the package '${packageName}' failed as there is no default catalog.`);
			continue;
		}
		const sub = catalogData.catalog[packageName];
		if (!sub) {
			logger.warn(`catalog: substitution for the package '${packageName}' failed as there is no matching package in the default catalog.`);
			continue;
		}
		...
		if (!catalog) {
			logger.warn(`'${version}' substitution for the package '${packageName}' failed as there is no matching catalog named '${catalogName}'. (available named catalogs are: ${Object.keys(catalogData.catalogs ?? {}).join(", ")})`);
			continue;
		}
		const sub = catalog[packageName];
		if (!sub) {
			logger.warn(`'${version}' substitution for the package '${packageName}' failed as there is no package in the catalog named '${catalogName}'. (packages in the catalog are: ${Object.keys(catalog).join(", ")})`);
			continue;
		}
		dependencies[packageName] = sub;
	}
...
const SOURCE_FILE_EXTENSIONS = /* @__PURE__ */ new Set([
	".ts",
	".tsx",
	".mts",
	".cts",
...
*/
function assertSchemasPathIsDirectory(path, option) {
	if (!namesAFile(path)) return;
	throw new Error(styleText("red", `\`${option}\` is a directory, but "${path}" names a file. Orval writes one file per schema into that directory, so a filename would become a directory with that name.\nPass the directory instead (e.g. \`${option}: './model'\`), or omit \`schemas\` to generate them alongside the target.`));
}
function normalizeSchemasOption(schemas, workspace) {
	...
	if (schemas.mode !== void 0 && schemas.mode !== "split" && schemas.mode !== "single") throw new Error("schemas.mode must be \"split\" or \"single\".");
	if (schemas.mode === "single") {
		if (schemas.type !== "zod") throw new Error("schemas.mode \"single\" requires schemas.type \"zod\".");
		if (schemas.splitByTags || schemas.routes) throw new Error("schemas.mode \"single\" cannot be combined with schemas.splitByTags or schemas.routes.");
	}
	if (schemas.mode !== "single") assertSchemasPathIsDirectory(schemas.path, "schemas.path");
	validatePackageSpecifier(schemas.importPath, "schemas.importPath");
	const routes = schemas.routes ? normalizeSchemaRoutes(schemas.routes) : void 0;
	return {
		...
		...schemas.mode === void 0 ? {} : { mode: schemas.mode },
		routes
	};
}
/** Normalizes the configured default and enum schema routes. */
function normalizeSchemaRoutes(routes) {
	if (!isString(routes.default)) throw new Error(styleText("red", "`schemas.routes.default` is required when `schemas.routes` is configured."));
	const normalized = {
		default: validateSchemaRoute(routes.default, "schemas.routes.default"),
		...routes.enum !== void 0 ? { enum: validateSchemaRoute(routes.enum, "schemas.routes.enum") } : {}
	};
	if (normalized.enum && normalized.enum.toLowerCase() === normalized.default.toLowerCase()) throw new Error(styleText("red", "`schemas.routes.default` and `schemas.routes.enum` must be different directories."));
	return normalized;
}
/** Validates and normalizes one schema route relative to `schemas.path`. */
function validateSchemaRoute(value, fieldName) {
	...
	const segments = portableRoute.split("/").filter(Boolean).filter((segment) => segment !== ".");
	if (route === "" || nodePath.isAbsolute(route) || isWindowsAbsolute || isUncAbsolute || segments.length === 0 || segments.includes("..")) throw new Error(styleText("red", `\`${fieldName}\` must be a relative directory under \`schemas.path\`.`));
	return upath.joinSafe(...segments);
}
/** Rejects schema-routing combinations that are unsupported by generation. */
function validateSchemaRoutes(output) {
	const schemas = output.schemas;
	if (!schemas || isString(schemas)) return;
	if (schemas.mode === "single") {
		if (output.operationSchemas || output.mock.generators.length > 0 || output.factoryMethods) throw new Error("schemas.mode \"single\" cannot be combined with operationSchemas, mock generators, or factoryMethods.");
	}
	if (!schemas.routes) return;
	if (output.operationSchemas) throw new Error(styleText("red", "`schemas.routes` cannot be used with `output.operationSchemas` in this release."));
	if (output.mock.generators.length > 0) throw new Error(styleText("red", "`schemas.routes` cannot be used with mock generators in this release."));
	if (output.factoryMethods) throw new Error(styleText("red", "`schemas.routes` cannot be used with `output.factoryMethods` in this release."));
}
/**
...
function validatePackageSpecifier(value, fieldName) {
	...
	if (nodePath.isAbsolute(value) || /^[A-Za-z]:[\\/]/.test(value) || value.startsWith("\\\\")) throw new Error(`\`${fieldName}\` must be a package specifier (e.g. '@acme/models'), not an absolute path. Received: "${value}"`);
}
function looksLikePackageSpecifier(value) {
	return !!value && value.trim() === value && !value.startsWith(".") && !nodePath.isAbsolute(value) && !/^[A-Za-z]:[\\/]/.test(value) && !value.startsWith("\\\\");
}
function resolvePackageSpecifier(workspace, value) {
	try {
		return createRequire(nodePath.join(workspace, "package.json")).resolve(value);
	} catch {
		return;
	}
}
function isPackageSpecifierCandidate(workspace, value) {
	...
	if (existsSync(nodePath.resolve(workspace, value))) return false;
	if (value.startsWith("@")) return true;
	const [packageName] = value.split("/");
	if (!value.includes("/")) return true;
	for (let dir = workspace;;) {
		if (existsSync(nodePath.join(dir, "node_modules", packageName))) return true;
...
*/
function normalizeAngularBaseUrl(baseUrl, outputClient, outputBaseUrl) {
	if (!baseUrl.apiId || !ANGULAR_BASE_URL_API_ID_REGEX.test(baseUrl.apiId)) throw new Error(styleText("red", `\`override.angular.baseUrl.apiId\` must be a non-empty string matching /^[A-Za-z][A-Za-z0-9_-]*$/ (got: ${JSON.stringify(baseUrl.apiId)}).`));
	if (outputBaseUrl !== void 0) throw new Error(styleText("red", "`override.angular.baseUrl` cannot be combined with the top-level `output.baseUrl`. Remove `output.baseUrl` — the base-URL token's server-URL fallback is resolved from the specification's `servers` field, or provide a custom resolver via the generated `provide<Api>BaseUrlResolver` helper."));
	if (outputClient !== OutputClient.ANGULAR) logger.warn(`\`override.angular.baseUrl\` is only supported by the \`angular\` client. It has no effect for other clients.`);
	return {
		apiId: baseUrl.apiId,
		...baseUrl.index !== void 0 ? { index: baseUrl.index } : {},
...
async function normalizeOptions(optionsExport, workspace = process.cwd(), globalOptions = {}) {
			...
			override: {
				...
				components: {
					schemas: {
						...
						itemPrefix: outputOptions.override?.components?.schemas?.itemPrefix ?? "",
					...
					parameters: {
						prefix: outputOptions.override?.components?.parameters?.prefix ?? "",
						suffix: RefComponentSuffix.parameters,
						...outputOptions.override?.components?.parameters
					},
					requestBodies: {
						prefix: outputOptions.override?.components?.requestBodies?.prefix ?? "",
				...
				zod: {
					...
					generateCompanionTypes: outputOptions.override?.zod?.generateCompanionTypes ?? false,
				...
				angular: {
					...
					runtimeValidation: normalizeRuntimeValidation(outputOptions.override?.angular?.runtimeValidation),
					...
					...outputOptions.override?.angular?.baseUrl ? { baseUrl: normalizeAngularBaseUrl(outputOptions.override.angular.baseUrl, outputOptions.client ?? client, outputOptions.baseUrl) } : {}
				},
				axios: { includeHttpResponseReturnType: outputOptions.override?.axios?.includeHttpResponseReturnType ?? false },
				fetch: {
					...
					serializeResponseHeaders: outputOptions.override?.fetch?.serializeResponseHeaders ?? false,
					runtimeValidation: normalizeRuntimeValidation(outputOptions.override?.fetch?.runtimeValidation),
				...
				useDates: (outputOptions.override?.useDates ?? false) || (outputOptions.override?.useDatesTransform ?? false),
				...
				includeZodSchemaInArguments: outputOptions.override?.includeZodSchemaInArguments ?? false
			},
	...
	if (normalizedOptions.output.httpClient === OutputHttpClient.FETCH && normalizedOptions.output.optionsParamRequired && normalizedOptions.output.override.requestOptions !== false) logger.warn(`With \`httpClient: 'fetch'\`, \`optionsParamRequired: true\` cannot make the generated \`options\` parameter required. The fetch \`options\` parameter remains optional with type \`RequestInit\` (\`optionsParamRequired\` may still affect other generated parameters). Set \`httpClient: 'axios'\` to make the \`options\` parameter required.`);
...
function normalizeMutator(workspace, mutator) {
	if (isObject(mutator)) {
		...
		return {
			path: !!resolvedPath || isPackageSpecifierCandidate(workspace, m.path) ? m.path : nodePath.resolve(workspace, m.path),
	...
	if (isString(mutator)) {
		...
		return {
			path: !!resolvedPath || isPackageSpecifierCandidate(workspace, mutator) ? mutator : nodePath.resolve(workspace, mutator),
			...resolvedPath ? { resolvedPath } : {},
			default: true
		};
	}
...
const supportedZodKeysLabel = `${operationZodOverrideKeys.slice(0, -1).join(", ")}, and ${operationZodOverrideKeys.at(-1)}`;
function normalizeOperationsAndTags(operationsOrTags, workspace, global, source) {
	return Object.fromEntries(Object.entries(operationsOrTags).map(([key, { transformer, mutator, formData, formUrlEncoded, paramsSerializer, paramsFilter, query, angular, zod, effect, ...rest }]) => {
		...
		if (unsupportedOperationZodKeys && unsupportedOperationZodKeys.length) {
			const fieldLabel = unsupportedOperationZodKeys.length === 1 ? "field" : "fields";
			const unsupportedFields = unsupportedOperationZodKeys.map((unsupportedKey) => `zod.${unsupportedKey}`).join(", ");
			logger.warn(`override.${source}.${key}.zod only supports ${supportedZodKeysLabel}. Ignoring unsupported ${fieldLabel}: ${unsupportedFields}.`);
		}
		const hasSupportedOperationZodConfig = !!zod && operationZodOverrideKeys.some((supportedKey) => zod[supportedKey] !== void 0);
		if (angular?.baseUrl) logger.warn(`override.${source}.${key}.angular.baseUrl is not supported — \`baseUrl\` is an output-level concern and is configured via \`override.angular.baseUrl\`. Ignoring.`);
		return [key, {
			...rest,
			...angular ? { angular: {
				provideIn: angular.provideIn ?? "root",
				client: angular.retrievalClient ?? angular.client ?? "httpClient",
				runtimeValidation: normalizeRuntimeValidation(angular.runtimeValidation),
...
function normalizeHonoOptions(hono = {}, workspace) {
	return {
		...hono.handlers ? { handlers: nodePath.resolve(workspace, hono.handlers) } : {},
		handlerGenerationStrategy: hono.handlerGenerationStrategy ?? "smart",
		compositeRoute: hono.compositeRoute ? nodePath.resolve(workspace, hono.compositeRoute) : "",
		validator: hono.validator ?? true,
		validatorOutputPath: hono.validatorOutputPath ? nodePath.resolve(workspace, hono.validatorOutputPath) : ""
	};
}
function normalizeMcpModuleOptions(module, workspace) {
	return {
...
function normalizeMcpOptions(mcp = {}, workspace) {
	return {
		...mcp.server && { server: normalizeMcpModuleOptions(mcp.server, workspace) },
		...mcp.handler && { handler: normalizeMcpModuleOptions(mcp.handler, workspace) }
	};
}
...
function normalizeQueryOptions(queryOptions = {}, outputWorkspace, globalOptions = {}) {
	if (queryOptions.options) logger.warn("Using query options is deprecated and will be removed in a future major release. Please use queryOptions or mutationOptions instead.");
	if (!isNullish(queryOptions.shouldExportQueryKey)) logger.warn("shouldExportQueryKey is deprecated and will be removed in a future major release. Please use shouldExportKeys instead, which also gates the mutation key getters.");
	const shouldExportKeys = queryOptions.shouldExportKeys ?? queryOptions.shouldExportQueryKey;
	return {
		...
		...isNullish(queryOptions.useSkipToken) ? {} : { useSkipToken: queryOptions.useSkipToken },
		...
		...isNullish(globalOptions.shouldExportKeys) ? {} : { shouldExportKeys: globalOptions.shouldExportKeys },
		...
		runtimeValidation: isNullish(queryOptions.runtimeValidation) ? globalOptions.runtimeValidation ?? normalizeRuntimeValidation(void 0) : normalizeRuntimeValidation(queryOptions.runtimeValidation)
	};
}
function getDefaultFilesHeader({ title, description, version } = {}) {
	return [
		`Generated by ${name} 🍺`,
...
async function resolveSpec(input, { parserOptions, transformer, workspace, unsafeDisableValidation = false }) {
	...
	if (!isWildcard) {
		...
		if (docs.length > 0) logger.warn(`External $ref documents being resolved:\n` + docs.map((d) => `  - ${d}`).join("\n"));
	}
	...
	const formDataItems = isSwagger2(transformedData) ? collectSwagger2FormDataItems(transformedData) : void 0;
	let specification = upgrade(transformedData).specification;
	specification = normalizeNullableRefs(specification);
	if (formDataItems && formDataItems.size > 0) specification = restoreSwagger2FormDataItems(specification, formDataItems);
...
*/
function normalizeNullableRefs(spec, path = [], inAllOf = false) {
	if (Array.isArray(spec)) return spec.map((item, i) => normalizeNullableRefs(item, [...path, String(i)], inAllOf));
	if (!isObject(spec)) return spec;
	const obj = spec;
	if (!inAllOf && "$ref" in obj && isString(obj.$ref) && obj.nullable === true) {
		const ref = obj.$ref;
		delete obj.nullable;
		delete obj.$ref;
		const jsonPointer = "#/" + path.join("/");
		logger.warn(`Invalid nullable reference found at ${jsonPointer}.\n  A \`$ref\` with a sibling \`nullable: true\` is out of spec (siblings on a reference are ignored).\n  Treating it as \`${ref} | null\` by rewriting to \`anyOf: [\`$ref\`, { type: "null" }]\`.\n  Fix the source spec with \`nullable: true\` + \`allOf: [$ref]\` (OpenAPI 3.0) or\n  \`anyOf: [$ref, { type: "null" }]\` (OpenAPI 3.1).`);
		return {
			anyOf: [{ $ref: ref }, { type: "null" }],
			...obj
		};
	}
	for (const [key, value] of Object.entries(obj)) obj[key] = normalizeNullableRefs(value, [...path, key], inAllOf || key === "allOf");
	return obj;
}
/**
...
*/
function paramKey(param, resolveParameter) {
	const resolved = resolveParameter(param);
	if (!resolved) return void 0;
	if (!isString(resolved.in) || !isString(resolved.name)) return;
	return `${resolved.in}:${resolved.name}`;
}
function isSwagger2(document) {
	return document.swagger === "2.0";
}
function collectSwagger2FormDataItems(document) {
	const reusableParameters = isObject(document.parameters) ? document.parameters : {};
	const resolveParameter = (param) => {
		if (!isObject(param)) return;
		if ("$ref" in param && isString(param.$ref)) {
			const ref = param.$ref;
			if (!ref.startsWith("#/parameters/")) return;
			const target = reusableParameters[ref.slice(13)];
			return isObject(target) ? target : void 0;
		}
		return param;
	};
	const formDataItems = /* @__PURE__ */ new Map();
	...
	for (const [path, pathItem] of Object.entries(paths)) {
		...
		for (const [method, operation] of Object.entries(pathItem)) {
			...
			const operationParams = Array.isArray(operation.parameters) ? operation.parameters : void 0;
			if (!operationParams && pathLevelParams.length === 0) continue;
			const merged = mergePathAndOperationParameters(pathLevelParams, operationParams, resolveParameter);
			const byName = /* @__PURE__ */ new Map();
			for (const rawParam of merged) {
				const param = resolveParameter(rawParam);
				if (!param || param.in !== "formData" || !isObject(param.items)) continue;
				byName.set(String(param.name), param.items);
			}
			const opKeys = new Set((operationParams ?? []).map((p) => paramKey(p, resolveParameter)).filter((k) => k !== void 0));
			const overriddenNames = /* @__PURE__ */ new Set();
			for (const rawParam of pathLevelParams) {
				const param = resolveParameter(rawParam);
				if (!param || param.in !== "formData" || !isString(param.name)) continue;
				const key = paramKey(rawParam, resolveParameter);
				if (key !== void 0 && opKeys.has(key)) overriddenNames.add(param.name);
			}
			if (byName.size > 0) {
...
*/
function mergePathAndOperationParameters(pathLevel, operationLevel, resolveParameter) {
	const opKeys = new Set((operationLevel ?? []).map((p) => paramKey(p, resolveParameter)).filter((k) => k !== void 0));
	return [...pathLevel.filter((p) => {
		const key = paramKey(p, resolveParameter);
		return key === void 0 || !opKeys.has(key);
	}), ...operationLevel ?? []];
}
/**
...
* already preserved.
*/
function restoreSwagger2FormDataItems(document, captured) {
	const paths = isObject(document.paths) ? document.paths : {};
	...
	const resolveRequestBody = (requestBody) => {
		...
		if ("$ref" in requestBody && isString(requestBody.$ref)) {
			...
			if (!ref.startsWith("#/components/requestBodies/")) return;
			const target = requestBodies[ref.slice(27)];
			return isObject(target) ? target : void 0;
		}
		return requestBody;
	};
	const patchProperties = (requestBody, byName) => {
		...
		for (const mediaType of Object.values(content)) {
			...
			for (const [paramName, items] of byName) {
				...
				if (!(propType === "array" || Array.isArray(propType) && propType.includes("array")) || property.items !== void 0) continue;
	...
	for (const [path, methods] of captured) {
		...
		for (const [method, capture] of methods) {
			...
			const effectiveBodies = /* @__PURE__ */ new Set();
			const directBody = resolveRequestBody(operation.requestBody);
			if (directBody) effectiveBodies.add(directBody);
			for (const rawParam of Array.isArray(operation.parameters) ? operation.parameters : []) {
				const body = resolveRequestBody(rawParam);
				if (body) effectiveBodies.add(body);
			}
			for (const body of effectiveBodies) patchProperties(body, byName);
			const sharedByNames = new Map([...byName].filter(([name]) => !overriddenNames.has(name)));
			for (const rawParam of Array.isArray(pathItem.parameters) ? pathItem.parameters : []) {
				const body = resolveRequestBody(rawParam);
				if (body) patchProperties(body, sharedByNames);
			}
		}
	}
	return document;
...
function resolveRefTarget(ref, origin) {
	...
	if (origin) return nodePath.resolve(nodePath.dirname(origin), doc);
...
function formatDisallowedRefsError(disallowed, currentAllowed) {
	...
	const all = [.../* @__PURE__ */ new Set([...currentAllowed, ...docs])];
...
function createSafeFileLoader(origin, isWildcard, allowedExternalRefs) {
	...
	return {
		...
		async exec(value) {
			...
			if (origin && nodePath.resolve(value) === nodePath.resolve(origin)) return base.exec(value);
...
* `$ref` could not.
*/
function isFetchableUrl(url, origin, allowedExternalRefs) {
	if (origin && resolveRefTarget(url, origin) === resolveRefTarget(origin)) return true;
...
*/
function createAllowListCheckedFetch(origin, allowedExternalRefs, headers) {
	return async (input, init) => {
		...
		for (let hop = 0;; hop++) {
			...
			if (hop >= MAX_EXTERNAL_REF_REDIRECTS) throw new Error(`Refused to follow more than ${MAX_EXTERNAL_REF_REDIRECTS} redirects while fetching an external $ref (stopped at ${url}).`);
			const next = new URL(location, url).href;
			if (!isFetchableUrl(next, origin, allowedExternalRefs)) {
				logger.warn(`Refused to follow a redirect to a URL that is not allowed: ${next}\nReached by redirect from ${url}.\nAdd it to externalRefs.allow or use ['*'] to allow all.`);
				throw new Error(`Refused to follow a redirect to a URL that is not allowed: ${next}`);
			}
			url = next;
		}
	};
...
function createSafeUrlLoader(origin, isWildcard, allowedExternalRefs, headers) {
	const base = fetchUrls({
		headers,
		...isWildcard ? {} : { fetch: createAllowListCheckedFetch(origin, allowedExternalRefs, headers) }
	});
	return {
		type: "loader",
		validate: base.validate,
...
*/
const schemaRefPrefix = (data) => isSwagger2(data) ? "#/definitions/" : "#/components/schemas/";
/**
...
*/
function dereferenceExternalRef(data, strategy = "default") {
	...
	const result = {};
	for (const [key, value] of Object.entries(data)) if (key !== "x-ext") result[key] = replaceXExtRefs(value, extensions, schemaNameMappings, refPrefix);
	return result;
}
/**
...
*/
function mergeExternalSchemas(data, extensions, strategy) {
	...
	const getMainSchemas = () => {
		...
		if (swagger2) {
			data.definitions ??= {};
			mainSchemas = data.definitions;
			return mainSchemas;
		}
		data.components ??= {};
		const mainComponents = data.components;
		mainComponents.schemas ??= {};
		mainSchemas = mainComponents.schemas;
		return mainSchemas;
	};
		...
		if (isObject(extDoc) && "components" in extDoc) {
			...
			if (isObject(extComponents) && "schemas" in extComponents) {
				...
				for (const [schemaName, schema] of Object.entries(extSchemas)) {
					...
					const isMatchingXExtRef = (isObject(existingSchema) && "$ref" in existingSchema && isString(existingSchema.$ref) && existingSchema.$ref) === `#/x-ext/${extKey}/components/schemas/${schemaName}`;
					let finalSchemaName = schemaName;
					const suffix = extKey.replaceAll(/[^a-zA-Z0-9]/g, "_");
					if (strategy === "always" && suffix.length === 0) throw new Error(`External schema "${schemaName}" from "${extKey}" cannot be named with the always strategy because its external document identity is empty after sanitization.`);
					if (strategy === "always") finalSchemaName = `${schemaName}_${suffix}`;
					else if (schemaName in targetSchemas && !isMatchingXExtRef) finalSchemaName = `${schemaName}_${suffix}`;
					const isExistingPlaceholder = finalSchemaName === schemaName && isMatchingXExtRef;
					if (Object.hasOwn(targetSchemas, finalSchemaName) && !isExistingPlaceholder) throw new Error(`External schema "${schemaName}" from "${extKey}" cannot be merged as "${finalSchemaName}" because that component name is already occupied.`);
	...
	for (const [extKey, mapping] of Object.entries(schemaNameMappings)) for (const [, finalName] of Object.entries(mapping)) {
		...
		if (schema) mainSchemas[finalName] = updateInternalRefs(schema, extKey, schemaNameMappings, refPrefix);
	}
...
function scrubUnwantedKeys(obj) {
	const UNWANTED_KEYS = /* @__PURE__ */ new Set(["$schema", "$id"]);
...
* (#2993).
*/
function updateInternalRefs(obj, extKey, schemaNameMappings, refPrefix) {
	if (obj === null || obj === void 0) return obj;
	if (Array.isArray(obj)) return obj.map((element) => updateInternalRefs(element, extKey, schemaNameMappings, refPrefix));
	if (isObject(obj)) {
		const record = obj;
		if ("$ref" in record && isString(record.$ref)) {
			...
			if (refValue.startsWith("#/components/schemas/")) {
				...
				if (mappedName) return { $ref: `${refPrefix}${mappedName}` };
			}
		}
		const result = {};
		for (const [key, value] of Object.entries(record)) result[key] = updateInternalRefs(value, extKey, schemaNameMappings, refPrefix);
		return result;
	}
	return obj;
}
...
*/
function replaceXExtRefs(obj, extensions, schemaNameMappings, refPrefix, inliningRefs = /* @__PURE__ */ new Set()) {
	if (isNullish$1(obj)) return obj;
	if (Array.isArray(obj)) return obj.map((element) => replaceXExtRefs(element, extensions, schemaNameMappings, refPrefix, inliningRefs));
	if (isObject(obj)) {
		const record = obj;
		if ("$ref" in record && isString(record.$ref)) {
			const refValue = record.$ref;
			if (refValue.startsWith("#/x-ext/")) {
				...
				if (extKey) {
					if (parts.length >= 3 && parts[0] === "components" && parts[1] === "schemas") {
						...
						return { $ref: `${refPrefix}${schemaNameMappings[extKey][schemaName] || schemaName}` };
					}
					if (inliningRefs.has(refValue)) {
						logger.warn(`Detected a circular external $ref while inlining "${refValue}". Replacing with an empty schema to avoid infinite recursion. Move the schema under "components.schemas" in its source file or pre-bundle the spec to keep the recursion intact.`);
						return {};
					}
					...
					if (refObj) {
						...
						return replaceXExtRefs(cleaned, extensions, schemaNameMappings, refPrefix, nextInlining);
					}
				}
			}
		}
		const result = {};
		for (const [key, value] of Object.entries(record)) result[key] = replaceXExtRefs(value, extensions, schemaNameMappings, refPrefix, inliningRefs);
		return result;
	}
	return obj;
}
...
async function createPrettierFileTransform() {
	...
	return async (filePath, content) => {
		try {
			...
			if (error instanceof Error && error.name === "UndefinedParserError") return content;
...
*/
async function formatWithPrettier(paths) {
	...
	if (format) {
		...
		await Promise.all(filePaths.map(async (filePath) => {
			try {
				...
				if (error instanceof Error && error.name === "UndefinedParserError") return;
				logger.warn(error, `Failed to format file ${filePath}`);
			}
		}));
		return;
	...
	try {
		...
		logger.warn("prettier not found. Install it as a project dependency or globally.");
	}
}
	...
	for (const p of paths) {
		...
		try {
			...
			if (stat.isFile()) results.push(absolute);
				...
				const subFiles = await collectFilePaths((await fs.readdir(absolute)).map((entry) => nodePath.join(absolute, entry)));
...
async function reExportSpecifierExists(indexFile, specifier, fileExtension, importExtension) {
	if (!specifier.startsWith(".")) return true;
	const resolved = nodePath.resolve(nodePath.dirname(indexFile), specifier);
	const candidates = /* @__PURE__ */ new Set([resolved]);
	if (importExtension && specifier.endsWith(importExtension)) candidates.add(resolved.slice(0, -importExtension.length) + fileExtension);
	candidates.add(`${resolved}${fileExtension}`);
	candidates.add(nodePath.join(resolved, `index${fileExtension}`));
	for (const candidate of candidates) if (await fs$2.pathExists(candidate)) return true;
	return false;
}
async function reconcileWorkspaceBarrel(filePath, specifiers, fileExtension, importExtension) {
	let existingContent;
	try {
		existingContent = await fs$2.readFile(filePath, "utf8");
	} catch (error) {
		if (error.code !== "ENOENT") throw error;
		await writeGeneratedFile(filePath, specifiers.map((s) => `export * from '${s}';`).join("\n") + "\n");
		return;
	}
	const declared = [...readReExportSpecifiers(existingContent)];
	const resolvable = await Promise.all(declared.map((s) => reExportSpecifierExists(filePath, s, fileExtension, importExtension)));
	const desired = new Set(specifiers);
	...
	const nextContent = [retained.join(eol).trimEnd(), [...desired].map((s) => `export * from '${s}';`).join(eol)].filter(Boolean).join(eol) + eol;
	if (nextContent !== existingContent) await writeGeneratedFile(filePath, nextContent);
}
async function reconcileZodBarrel(filePath, specifiers, header, mergeExisting) {
	let existingContent = "";
	try {
		existingContent = await fs$2.readFile(filePath, "utf8");
	} catch (error) {
	...
	const existing = mergeExisting ? readReExportSpecifiers(existingContent) : /* @__PURE__ */ new Set();
	const nextContent = `${header}\n${[.../* @__PURE__ */ new Set([...existing, ...specifiers])].sort().map((s) => `export * from '${s}';`).join("\n")}\n`;
	if (nextContent !== existingContent) await writeGeneratedFile(filePath, nextContent);
}
//#endregion
...
async function startWatcher(watchOptions, watchFn, defaultTarget = ".") {
	...
	logger.info(`Watching for changes in ${Array.isArray(watchPaths) ? watchPaths.map((v) => "\"" + v + "\"").join(" | ") : "\"" + watchPaths + "\""}`);
...
const generateReusableSchemaSet = (refs, context, options) => {
	...
	for (const ref of queue) {
		...
		const definition = generateZodValidationSchemaDefinition(schema, scopedContext, name, options.strict, options.isZodV4, {
			required: true,
			useReusableSchemas: true,
			emitMeta: options.generateMeta
		});
		const parsed = parseZodValidationSchemaDefinition(definition, scopedContext, options.coerce ?? false, options.strict, options.isZodV4, void 0, options.paramsMutator ? {
...
function computeCrossDirImportPath(schemasPath, fromDir, toDir, fileName, importExt) {
	...
	const fromPath = fromDir === ROOT_DIR ? schemasPath : nodePath.join(schemasPath, fromDir);
	const toPath = toDir === ROOT_DIR ? schemasPath : nodePath.join(schemasPath, toDir);
...
function renderReusableSchemaEntry(entry, context, zodVariant) {
	...
	if (entry.isRecursive) {
		...
		return {
			content: consts + subModelBlock + renderZodExport({
				name: entry.name,
	...
	return {
		content: consts + renderZodExport({
			name: entry.name,
			expression: `${entry.zod};`,
			variant: zodVariant,
			companionTypes: true
...
*/
function buildSiblingImports({ usedRefs, extraImports, entryName, componentNames, namingConvention, importExt, schemaTagMap, schemaOutputPlan, currentDir, schemasPath }) {
	...
	return [...importsByName.values()].toSorted((a, b) => a.name.localeCompare(b.name)).map(({ name, alias }) => {
		...
		return `import { ${alias ? `${name} as ${alias}` : name} } from '${schemaOutputPlan ? schemaOutputPlan.importPathFor(entryName, name) : schemaTagMap && currentDir && schemasPath ? computeCrossDirImportPath(schemasPath, currentDir, getSchemaDir(schemaTagMap, name), importedFile, importExt) : `./${importedFile}${importExt}`}';`;
	}).join("\n");
}
...
const groupSchemasByFilePath = (schemas) => {
	...
	return [...grouped.values()].map((group) => [...group].toSorted((a, b) => compareNatural(a.filePath, b.filePath))).toSorted((a, b) => compareNatural(a[0].filePath, b[0].filePath));
};
/** Resolves a Zod schema file path from the route plan or legacy layout. */
function getZodSchemaFilePath(name, schemasPath, fileExtension, namingConvention, schemaOutputPlan, schemaTagMap, scopeTag) {
	if (schemaOutputPlan) {
		const existing = schemaOutputPlan.filePathByName.get(name);
		if (existing) return existing;
		const tag = scopeTag ?? schemaTagMap?.get(name);
		const scopePath = tag ? nodePath.join(schemaOutputPlan.routePathByKey.default, tag === "." ? "shared" : tag) : schemaOutputPlan.routePathByKey.default;
		return schemaOutputPlan.registerSchema(name, "default", scopePath);
	}
	return nodePath.join(schemasPath, `${conventionName(name, namingConvention)}${fileExtension}`);
}
async function writeZodSchemaIndex(schemasPath, fileExtension, header, schemaNames, namingConvention, shouldMergeExisting = false, tsconfig) {
	...
	await reconcileZodBarrel(nodePath.join(schemasPath, `index.ts`), [...new Set(schemaNames.map((schemaName) => {
		return `./${conventionName(schemaName, namingConvention)}${importFileExtension}`;
	}))], header, shouldMergeExisting);
}
/** Writes a generated Zod barrel without merging unrelated existing exports. */
async function writeZodBarrel(indexPath, specifiers, header) {
...
async function writeZodSchemaRoutesBarrel(plan, fileExtension, header, schemaNames, namingConvention, tsconfig) {
	const namesByScope = /* @__PURE__ */ new Map();
	for (const name of schemaNames) {
		const scope = plan.scopePathByName.get(name) ?? plan.routePathByName.get(name);
		if (!scope) continue;
	...
	for (const [scopePath, names] of namesByScope) await writeZodSchemaIndex(scopePath, fileExtension, header, names, namingConvention, false, tsconfig);
	const scopesByRoute = /* @__PURE__ */ new Map();
	...
	const routeExports = [];
	for (const route of ["default", "enum"]) {
		const routePath = plan.routePathByKey[route];
		const scopes = scopesByRoute.get(route) ?? [];
		if (scopes.length === 0) continue;
		if (plan.usesTagRouting) {
			const scopeExports = scopes.map((scopePath) => {
				return `./${upath.toUnix(nodePath.relative(routePath, scopePath))}/index${indexImportExt}`;
			}).toSorted((a, b) => compareNatural(a, b));
			await writeZodBarrel(nodePath.join(routePath, "index.ts"), scopeExports, header);
		}
		const routeName = upath.toUnix(nodePath.relative(plan.basePath, routePath));
		routeExports.push(`./${routeName}/index${indexImportExt}`);
	}
	await writeZodBarrel(nodePath.join(plan.basePath, "index.ts"), routeExports, header);
}
async function writeZodSchemaTagsSplitBarrel(schemasPath, fileExtension, header, componentDirs, verbDirs, namingConvention, tsconfig) {
	...
	for (const [dir, schemaNames] of allDirs) {
		...
		await writeZodSchemaIndex(nodePath.join(schemasPath, dir), fileExtension, header, schemaNames, namingConvention, false, tsconfig);
	}
	...
	const tagExports = [...allDirs.keys()].filter((dir) => dir !== ROOT_DIR).toSorted((a, b) => compareNatural(a, b)).map((dir) => {
		return `export * from '${indexImportExt ? `./${dir}/index${indexImportExt}` : `./${dir}`}';`;
	});
	const allExports = [...rootExports, ...tagExports];
	const rootIndexPath = nodePath.join(schemasPath, "index.ts");
...
function generateZodSchemasInline(builder, output, includeZodImport = true, paramsMutator, includeParamsImport = false) {
	...
	for (const { name, schema: schemaObject } of schemasWithOpenApiDef) {
		...
		const zodDefinition = generateZodValidationSchemaDefinition(dereferencedSchema, context, name, strict, isZodV4, {
			required: true,
			emitMeta: output.override.zod.generateMeta
		});
		const parsedZodDefinition = parseZodValidationSchemaDefinition(zodDefinition, context, coerce, strict, isZodV4, void 0, void 0, output.override.zod.variant, output.override.zod.exactOptional);
		schemas.push({
			schemaName: name,
...
function generateZodSchemasInlineReusable(builder, output, includeZodImport = true, paramsMutator, includeParamsImport = false) {
	...
	const body = rewriteReusableSchemas(entries).map((entry) => renderReusableSchemaEntry(entry, context, output.override.zod.variant).content).join("\n\n");
...
async function writeZodSchemas(builder, schemasPath, fileExtension, header, output, paramsMutator, schemaTagMap, schemaOutputPlan) {
	if (output.override.zod.generateReusableSchemas) return writeZodSchemasReusable(builder, schemasPath, fileExtension, header, output, paramsMutator, schemaTagMap, schemaOutputPlan);
	...
	for (const generatorSchema of schemasWithOpenApiDef) {
		...
		const filePath = schemaOutputPlan ? getZodSchemaFilePath(name, schemasPath, fileExtension, output.namingConvention, schemaOutputPlan, schemaTagMap) : isSplit ? nodePath.join(schemasPath, tagDir, `${fileName}${fileExtension}`) : nodePath.join(schemasPath, `${fileName}${fileExtension}`);
		const context = {
			spec: builder.spec,
			target: builder.target,
			workspace: "",
	...
	if (output.indexFiles && !isSplit && !schemaOutputPlan) await writeZodSchemaIndex(schemasPath, fileExtension, header, writtenSchemaNames, output.namingConvention, false, output.tsconfig);
	if (isSplit) {
	...
	return /* @__PURE__ */ new Map([[ROOT_DIR, writtenSchemaNames]]);
}
/** Writes reusable Zod schemas while preserving their planned route scopes. */
async function writeZodSchemasReusable(builder, schemasPath, fileExtension, header, output, paramsMutator, schemaTagMap, schemaOutputPlan) {
	const isSplit = !!schemaTagMap;
	const isZodV4 = resolveIsZodV4(output.override.zod.version, output.packageJson);
	assertZodTarget({
		variant: output.override.zod.variant,
	...
	for (const entry of rewritten) {
		...
		const filePath = schemaOutputPlan ? getZodSchemaFilePath(entry.name, schemasPath, fileExtension, output.namingConvention, schemaOutputPlan, isSplit ? schemaTagMap : void 0, isSplit ? tagDir : void 0) : isSplit ? nodePath.join(schemasPath, tagDir, `${fileName}${fileExtension}`) : nodePath.join(schemasPath, `${fileName}${fileExtension}`);
	...
	if (output.indexFiles && !isSplit && rewritten.length > 0 && !schemaOutputPlan) await writeZodSchemaIndex(schemasPath, fileExtension, header, rewritten.map((e) => e.name), output.namingConvention, true, output.tsconfig);
	if (isSplit) {
		const dirSchemas = /* @__PURE__ */ new Map();
		for (const entry of rewritten) {
	...
	return /* @__PURE__ */ new Map([[ROOT_DIR, rewritten.map((e) => e.name)]]);
}
/** Builds operation schemas for either single-file or split-file output. */
function generateZodSchemasFromVerbs(verbOptions, schemasPath, fileExtension, output, context, schemaTagMap, schemaOutputPlan) {
	const isSplit = !!schemaTagMap;
	...
	const generateVerbsSchemas = verbOptionsArray.flatMap((verbOption) => {
		...
		const resolvedParameters = operation.parameters?.map((p) => isReference(p) && typeof p.$ref === "string" ? resolveRef(p, zodContext).schema : p);
		const pathParams = resolvedParameters?.filter((p) => "in" in p && p.in === "path");
		...
		const queryParams = resolvedParameters?.filter((p) => "in" in p && p.in === "query");
		...
		const headerParams = resolvedParameters?.filter((p) => "in" in p && p.in === "header");
		...
		const responseSchemas = shouldGenerate.response ? [...verbOption.response.types.success, ...verbOption.response.types.errors].map((responseType) => {
			const cleanName = responseType.value.replace(/(\[\])+$/, "");
			const hadArraySuffix = cleanName !== responseType.value;
			let cleanSchema = responseType.originalSchema;
			if (hadArraySuffix) {
				while (cleanSchema && "type" in cleanSchema && cleanSchema.type === "array" && cleanSchema.items && !isReference(cleanSchema.items) && !("$ref" in cleanSchema.items)) cleanSchema = cleanSchema.items;
				if (!cleanSchema || "type" in cleanSchema && cleanSchema.type === "array") cleanSchema = void 0;
			}
			return {
		...
		}).filter((entry) => !!entry.cleanSchema && !entry.isRef && isValidSchemaIdentifier(entry.cleanName) && !isPrimitiveSchemaName(entry.cleanName)).map((entry) => ({
			name: entry.cleanName,
			schema: useReusableSchemas ? entry.cleanSchema : dereference(entry.cleanSchema, zodContext)
		})) : [];
	...
	for (const entry of uniqueVerbsSchemas) {
		...
		const filePath = schemaOutputPlan ? getZodSchemaFilePath(name, schemasPath, fileExtension, output.namingConvention, schemaOutputPlan, isSplit ? schemaTagMap : void 0, isSplit ? tagDir : void 0) : isSplit ? nodePath.join(schemasPath, tagDir, `${fileName}${fileExtension}`) : nodePath.join(schemasPath, `${fileName}${fileExtension}`);
		const zodDefinition = "bodyContentType" in entry && entry.bodyContentType === "multipart/form-data" ? generateFormDataZodSchema(schema, zodContext, name, strict, isZodV4, "encoding" in entry ? entry.encoding : void 0, useReusableSchemas) : generateZodValidationSchemaDefinition(schema, zodContext, name, strict, isZodV4, {
			required: true,
			useReusableSchemas
		});
		const parsedZodDefinition = parseZodValidationSchemaDefinition(zodDefinition, zodContext, coerce, strict, isZodV4, void 0, void 0, output.override.zod.variant, output.override.zod.exactOptional);
		...
		if (useReusableSchemas && parsedZodDefinition.usedRefs.size > 0) {
			...
			importStatements = [...parsedZodDefinition.usedRefs].filter((refName) => refName !== name).toSorted().map((refName) => {
				...
				return `import { ${refName} } from '${schemaOutputPlan ? schemaOutputPlan.importPathFor(name, refName) : isSplit ? computeCrossDirImportPath(schemasPath, tagDir, getSchemaDir(schemaTagMap, refName), importedFile, importExt) : `./${importedFile}${importExt}`}';`;
			});
		}
		schemasToWrite.push({
			schemaName: name,
...
async function writeZodSchemasFromVerbs(verbOptions, schemasPath, fileExtension, header, output, context, schemaTagMap, schemaOutputPlan) {
	const isSplit = !!schemaTagMap;
	const useReusableSchemas = output.override.zod.generateReusableSchemas === true;
	const { schemasToWrite, uniqueVerbsSchemas } = generateZodSchemasFromVerbs(verbOptions, schemasPath, fileExtension, output, context, schemaTagMap, schemaOutputPlan);
	if (Object.keys(verbOptions).length === 0) return /* @__PURE__ */ new Map();
	const groupedSchemasToWrite = groupSchemasByFilePath(schemasToWrite);
	for (const schemaGroup of groupedSchemasToWrite) {
		const fileContent = generateZodSchemaFileContent(header, schemaGroup, output.override.zod.variant);
		await writeGeneratedFile(schemaGroup[0].filePath, fileContent);
	...
	const writtenSchemaNames = groupedSchemasToWrite.map((schemaGroup) => schemaGroup[0].schemaName);
	if (output.indexFiles && !isSplit && uniqueVerbsSchemas.length > 0 && !schemaOutputPlan) await writeZodSchemaIndex(schemasPath, fileExtension, header, writtenSchemaNames, output.namingConvention, true, output.tsconfig);
	if (isSplit) {
		const dirSchemas = /* @__PURE__ */ new Map();
		for (const entry of uniqueVerbsSchemas) {
			if (useReusableSchemas && entry.schema && typeof entry.schema.$ref === "string" && Object.keys(entry.schema).length === 1) continue;
...
async function writeZodSchemasSingle(builder, verbOptions, schemasPath, fileExtension, header, output, context, paramsMutator) {
	const { schemasToWrite } = generateZodSchemasFromVerbs(verbOptions, schemasPath, fileExtension, output, context);
	const operationNames = new Set(schemasToWrite.map((entry) => entry.schemaName));
	const components = generateZodSchemasInline({
		...builder,
		schemas: builder.schemas.filter((schema) => !operationNames.has(schema.name))
	}, output, false, paramsMutator, true);
	const operations = generateZodSchemaFileContent("", schemasToWrite.map((entry) => ({
		...entry,
		importStatements: void 0
	})), output.override.zod.variant, false);
	await writeGeneratedFile(namesAFile(schemasPath) ? schemasPath : nodePath.join(schemasPath, `index${fileExtension}`), `${header}${getZodSchemaImportStatement(output.override.zod.variant)}\n\n${components}${operations}`);
}
//#endregion
//#region src/write-specs.ts
async function runExternalFormatter(bin, args) {
	try {
		await execa(bin, args);
	} catch (error) {
		if (error instanceof ExecaError && error.code === "ENOENT") logger.warn(`${bin} not found`);
		else logger.warn(error);
	}
}
async function runFormatter(formatter, paths) {
	switch (formatter) {
		...
		case SupportedFormatter.BIOME:
			...
			break;
		case SupportedFormatter.OXFMT: await runExternalFormatter(SupportedFormatter.OXFMT, paths);
	}
}
const DOCS_MARKDOWN_PLUGIN = "typedoc-plugin-markdown";
const DOCS_MARKDOWN_THEME = "markdown";
function getDocsTypedocOptions(entryPoints, config) {
	const plugin = config.plugin ?? [];
	return {
		entryPoints,
		skipErrorChecking: true,
		...config,
		plugin: plugin.includes(DOCS_MARKDOWN_PLUGIN) ? plugin : [DOCS_MARKDOWN_PLUGIN, ...plugin]
	};
}
/**
...
*/
function getDocsOutputName(theme) {
...
*/
function createMarkdownPluginReader() {
	return {
		name: "orval-ensure-markdown-plugin",
		order: Number.MAX_SAFE_INTEGER,
		supportsPackages: false,
		read(container) {
			const current = container.isSet("plugin") ? container.getValue("plugin") : [];
			if (!current.includes(DOCS_MARKDOWN_PLUGIN)) container.setValue("plugin", [...current, DOCS_MARKDOWN_PLUGIN]);
		}
	};
}
...
async function addOperationSchemasReExport(schemaPath, operationSchemasPath, header) {
	const schemaIndexPath = nodePath.join(schemaPath, `index.ts`);
	...
	let existingContent;
	try {
		existingContent = await fs$2.readFile(schemaIndexPath, "utf8");
	} catch (error) {
		if (error.code !== "ENOENT") throw error;
	}
	if (existingContent !== void 0) {
		if (!readReExportSpecifiers(existingContent).has(esmImportPath)) await writeGeneratedFile(schemaIndexPath, existingContent + exportLine);
	} else {
...
async function writeFakerSchemaMocks(builder, options, header, schemaTagMap) {
	...
	const { implementation, imports, strictMockSchemaTypeNames, strictMockSchemaKinds } = generateFakerForSchemas(schemasWithDef, context, fakerEntry);
	...
	if (output.schemas) {
		...
		filePath = nodePath.join(schemasDir, `index.faker${fileExtension}`);
		schemaImportPath = ".";
	} else {
		...
		filePath = nodePath.join(dir, `schemas.faker${fileExtension}`);
...
function getImplementationPathsForIndex(output, implementationPaths, indexFile) {
	...
	if (shouldExcludeSelf && output.mode === OutputMode.SPLIT && getComparableFilePath(output.target) === getComparableFilePath(indexFile)) {
		...
		return excludeFilePath(paths, nodePath.join(targetInfo.dirname, `${targetInfo.filename}.schemas${output.fileExtension}`));
	}
	if (!(output.mode === OutputMode.TAGS_OPERATIONS || output.mode === OutputMode.TAGS_OPERATIONS_SPLIT) || !shouldExcludeSelf) return paths;
	const targetInfo = getFileInfo(output.target, { extension: output.fileExtension });
	const rootBarrel = nodePath.join(targetInfo.dirname, `index${output.fileExtension}`);
	const globalSchemas = nodePath.join(targetInfo.dirname, `${targetInfo.filename}.schemas${output.fileExtension}`);
...
async function writeSpecsInternal(builder, workspace, options, projectName) {
	const { info, schemas, target } = builder;
	const { output } = options;
	const { schemaTagMap, schemaOutputPlan } = builder;
	const shouldSplitSchemasByTags = isObject(output.schemas) && output.schemas.splitByTags === true;
	...
	if (output.schemas) {
		...
		const singleZodFile = isObject(output.schemas) && output.schemas.mode === "single";
		...
		if (isZodSchemas) {
			...
			const schemasDir = singleZodFile && namesAFile(schemasPath) ? nodePath.dirname(schemasPath) : schemasPath;
			const schemasParamsMutator = output.override.zod.params ? await generateMutator({
				output: nodePath.join(schemasDir, `__params__${fileExtension}`),
				mutator: output.override.zod.params,
				name: "zodParams",
				workspace,
				tsconfig: output.tsconfig
			}) : void 0;
			if (singleZodFile) await writeZodSchemasSingle(builder, builder.verbOptions, schemasPath, fileExtension, header, output, {
				spec: builder.spec,
				target: builder.target,
				workspace,
				output
				...
				const componentDirs = await writeZodSchemas(builder, schemasPath, fileExtension, header, output, schemasParamsMutator, schemaTagMap, schemaOutputPlan);
				...
				if (output.indexFiles) {
					if (schemaOutputPlan) await writeZodSchemaRoutesBarrel(schemaOutputPlan, fileExtension, header, [...componentDirs.values(), ...verbDirs.values()].flat(), output.namingConvention, output.tsconfig);
					else await writeZodSchemaTagsSplitBarrel(schemasPath, fileExtension, header, componentDirs, verbDirs, output.namingConvention, output.tsconfig);
				}
			} else {
				const componentDirs = await writeZodSchemas(builder, schemasPath, fileExtension, header, output, schemasParamsMutator, void 0, schemaOutputPlan);
				const verbDirs = await writeZodSchemasFromVerbs(builder.verbOptions, schemasPath, fileExtension, header, output, {
					spec: builder.spec,
					target: builder.target,
					workspace,
					output
				}, void 0, schemaOutputPlan);
				if (schemaOutputPlan && output.indexFiles) await writeZodSchemaRoutesBarrel(schemaOutputPlan, fileExtension, header, [...componentDirs.values(), ...verbDirs.values()].flat(), output.namingConvention, output.tsconfig);
			}
		} else {
			const fileExtension = output.fileExtension || ".ts";
			if (schemaOutputPlan) await writeRoutedSchemas({
				...
				if (!schemaTagMap) throw new Error("schemas.splitByTags is enabled but no schema tag map was built. The map comes from getApiBuilder, so a WriteSpecBuilder assembled another way has to carry one.");
	...
	if (output.target) {
		...
		const operationsUseZod = Object.values(builder.operations).some((operation) => /\bzod\b/.test(operation.implementation));
	...
	if (output.workspace) {
		...
		const indexFile = nodePath.join(workspacePath, "index.ts");
		...
		const imports = getImplementationPathsForIndex(output, implementationPaths, indexFile).filter((p) => mockExtensions.length === 0 || !mockExtensions.some((ext) => p.endsWith(`.${ext}.ts`))).map((p) => {
			...
			return stripFileExtension(relative, output.fileExtension) + importExtension;
		});
		if (output.schemas) {
			const schemasPath = isString(output.schemas) ? output.schemas : output.schemas.path;
			const isNamedSingleSchema = isObject(output.schemas) && output.schemas.mode === "single" && namesAFile(schemasPath);
			if (schemaOutputPlan) imports.push(upath.getRelativeImportPath(indexFile, schemaOutputPlan.basePath));
			else if (isNamedSingleSchema) {
				const schemaSourceExtension = nodePath.extname(schemasPath);
				const schemaImportExtension = getImportExtension(schemaSourceExtension, output.tsconfig);
				const relative = upath.getRelativeImportPath(indexFile, schemasPath, true);
				imports.push(stripFileExtension(relative, schemaSourceExtension) + schemaImportExtension);
			} else imports.push(upath.getRelativeImportPath(indexFile, getFileInfo(schemasPath).dirname));
		}
		...
		if (output.indexFiles) {
			await reconcileWorkspaceBarrel(indexFile, [...new Set(imports)], output.fileExtension, importExtension);
			implementationPaths = [indexFile, ...excludeFilePath(implementationPaths, indexFile)];
		}
	}
	if (builder.extraFiles.length > 0) {
		await Promise.all(builder.extraFiles.map(async (file) => writeGeneratedFile(file.path, file.content)));
	...
	const paths = [
		...schemaOutputPlan ? [
			...
			...schemaOutputPlan.rootIndexPath ? [schemaOutputPlan.rootIndexPath] : []
		] : output.schemas ? [!isString(output.schemas) && output.schemas.mode === "single" && namesAFile(output.schemas.path) ? output.schemas.path : getFileInfo(isString(output.schemas) ? output.schemas : output.schemas.path).dirname] : [],
	...
	if (output.docs) try {
		...
		const app = await Application.bootstrapWithPlugins(getDocsTypedocOptions(paths.map((x) => upath.toUnix(x)), config), [
			new TypeDocReader(),
			new PackageJsonReader(),
			new TSConfigReader(),
			createMarkdownPluginReader()
		]);
		if (!app.options.isSet("theme")) app.options.setValue("theme", DOCS_MARKDOWN_THEME);
		...
		if (project) {
			...
			app.outputs.setDefaultOutputName(getDocsOutputName(app.options.getValue("theme")));
...
async function writeSpecs(builder, workspace, options, projectName) {
	const transform = options.output.formatter === SupportedFormatter.PRETTIER && !options.hooks.afterAllFilesWrite ? await createPrettierFileTransform() : void 0;
	if (transform) return withGeneratedFileTransform(transform, () => writeSpecsInternal(builder, workspace, options, projectName));
...
const toComparablePath = (directory) => upath.resolve(directory);
/**
...
const isBelow = (parent, child) => {
	...
	return childSegments.length > parentSegments.length && parentSegments.every((segment, index) => segment === childSegments[index]);
};
/** The path of `child` relative to `parent`. Requires `isBelow(parent, child)`. */
const relativeToParent = (parent, child) => child.split("/").slice(parent.split("/").length).join("/");
/**
...
async function generateSpec(workspace, options, projectName) {
	if (options.output.clean) {
		...
		const ownedDirectories = /* @__PURE__ */ new Set();
		if (options.output.target) ownedDirectories.add(toComparablePath(getFileInfo(options.output.target).dirname));
		if (options.output.schemas) {
			const schemas = options.output.schemas;
			if (!isString(schemas) && schemas.mode === "single" && namesAFile(schemas.path)) {
				const schemasDirectory = getFileInfo(schemas.path).dirname;
				await Promise.all([fs.rm(schemas.path, { force: true }), fs.rm(upath.join(schemasDirectory, `__params__${options.output.schemaFileExtension}`), { force: true })]);
			} else ownedDirectories.add(toComparablePath(isString(schemas) ? schemas : schemas.path));
		}
		const mockDirectories = getConfiguredMockDirectories(options.output.mock).map(toComparablePath);
		for (const directory of ownedDirectories) {
			const nestedMockPatterns = mockDirectories.filter((mockDirectory) => isBelow(directory, mockDirectory)).map((mockDirectory) => `!${escapePath(relativeToParent(directory, mockDirectory))}/**`);
		...
		const mockPatterns = Object.values(OutputMockType).flatMap((type) => [.../* @__PURE__ */ new Set([options.output.fileExtension, ...MOCK_FILE_EXTENSIONS])].map((extension) => `**/*.${type}${extension}`));
		for (const directory of mockDirectories) {
			...
			if (mockRootStats.isSymbolicLink()) {
				logger.warn(`Skipped cleaning "${directory}": the configured mock path is a symbolic link.`);
				continue;
			}
			await removeFilesAndEmptyFolders(mockPatterns, directory, { followSymbolicLinks: false });
		}
...
function findConfigFile(configFilePath) {
	if (configFilePath) {
		const absolutePath = nodePath.isAbsolute(configFilePath) ? configFilePath : nodePath.resolve(process.cwd(), configFilePath);
	...
	for (const ext of [
		...
		const fullPath = nodePath.resolve(root, `orval.config${ext}`);
...
//#endregion
export { defineConfig as a, logger as c, version as d, startWatcher as i, description as l, loadConfigFile as n, defineTransformer as o, generateSpec as r, normalizeOptions as s, findConfigFile as t, name as u };

//# sourceMappingURL=config-l1dkzOlS.mjs.map