import { Verbs, createLogger, generateBodyOptions, generateFormDataAndUrlEncodedFunction, generateVerbImports, getIsBodyVerb, isObject, isOperationInTagBucket, jsStringLiteralEscape, pascal, resolveRef, sanitize, toObjectString } from "@orval/core";
//#region src/logger.ts
const logger = createLogger("@orval/solid-start");
//#endregion
//#region src/index.ts
...
const resolveNamespaceName = (title, verbOptions, tag) => {
	...
	logger.warn(`solid-start: exporting the namespace as \`${candidate}\` instead of \`${title}\`, because this file also imports a schema named \`${title}\`. Import \`${candidate}\` from this file, or rename the schema to keep \`${title}\`.`);
	return candidate;
};
