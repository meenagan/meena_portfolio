...
const renderArrayWithBounds = (itemName, variant, rules) => {
	if (variant === "mini") {
		...
		return `${zodMiniCall("array", itemName)}${checks.map((check) => `.check(${check})`).join("")}`;
	}
	return `zod.array(${itemName})${rules?.min ? `.min(${rules.min})` : ""}${rules?.max ? `.max(${rules.max})` : ""}`;
};
const renderBrand = (block) => {
	if (!block.brand) return "";
	return block.brand.isZodV4 ? `.brand("${block.name}")` : `.brand<"${block.name}">()`;
};
/**
...
const renderZodExport = (block) => {
	...
	if (!block.companionTypes) return constLine;
	return `${constLine}\n\n${companionLines(block.name)}`;
};
//#endregion
//#region src/index.ts
...
*/
function needsInlineDefault(schema, seen = /* @__PURE__ */ new Set()) {
	if (!schema || typeof schema !== "object") return false;
	if (seen.has(schema)) return false;
	seen.add(schema);
	const node = schema;
	if ("prefixItems" in node) return true;
	const items = node.items;
	if (items && typeof items === "object") {
		if ("$ref" in items) return true;
		if (needsInlineDefault(items, seen)) return true;
	}
	const properties = node.properties;
	if (properties && typeof properties === "object") return Object.values(properties).some((child) => needsInlineDefault(child, seen));
	return false;
}
...
const generateZodValidationSchemaDefinition = (schema, context, name, strict, isZodV4, rules) => {
	...
	const applyChainableSiblings = (functions, consts, siblingSchema) => {
		...
		if (typeof siblingSchema.description === "string") functions.push(["describe", `'${jsStringLiteralEscape(siblingSchema.description)}'`]);
	};
	if (rules?.useReusableSchemas && "$ref" in schema && typeof schema.$ref === "string" && isComponentSchemaRef(schema.$ref)) {
		...
		logger.verbose(`$ref ${schema.$ref} has non-chainable siblings [${siblings.filter((s) => !isChainable(s)).join(", ")}]; falling back to inlining.`);
	...
	if (rules?.useReusableSchemas && isDynamicReference(schema)) {
		...
		if (anchorName) {
			...
			if (resolvedTypeName !== "unknown" && schemaName) {
				...
				logger.verbose(`$dynamicRef ${schema.$dynamicRef} has non-chainable siblings [${siblings.filter((s) => !isChainable(s)).join(", ")}]; falling back to inlining.`);
			}
		}
		schema = dereference(schema, context);
	}
	...
	const pushDescriptionOrMeta = () => {
		...
		if (rules?.emitMeta && isZodV4) {
			...
			if (deprecated) meta.deprecated = true;
			functions.push(["meta", meta]);
		} else if (description !== void 0) functions.push(["describe", `'${jsStringLiteralEscape(description)}'`]);
	};
	...
	if (schema.default !== void 0) {
		...
		if (schema.type === "string" && (schema.format === "date" || schema.format === "date-time") && context.output.override.useDates) defaultValue = `new Date(${JSON.stringify(schema.default)})`;
			...
			const formatDefaultEntryValue = (entryValue, entrySchema) => {
				if (isString(entryValue)) return `${JSON.stringify(entryValue)} as const`;
				if (Array.isArray(entryValue)) {
					const entrySchemaObj = entrySchema && typeof entrySchema === "object" ? entrySchema : void 0;
					if (entrySchemaObj && Array.isArray(entrySchemaObj.prefixItems) ? entrySchemaObj.prefixItems : void 0) return `[${entryValue.map((item) => {
						if (isString(item)) return `${JSON.stringify(item)} as const`;
						if (isNumber(item) || isBoolean(item)) return `${item}`;
						if (item === null || item === void 0) return `${item}`;
						return formatDefaultEntryValue(item) ?? "null";
					}).join(", ")}]`;
					const itemsSchema = entrySchemaObj?.items;
					return `[${entryValue.map((item) => {
						if (isString(item)) return `${JSON.stringify(item)} as const`;
						if (isNumber(item)) return `${item} as const`;
						if (Array.isArray(item)) return formatDefaultEntryValue(item, itemsSchema) ?? "[]";
						if (isObject(item)) return formatDefaultEntryValue(item, itemsSchema) ?? "null";
						return `${JSON.stringify(item)}`;
					}).join(", ")}]`;
				}
				if (isObject(entryValue)) {
					const nestedEntries = Object.entries(entryValue).map(([nestedKey, nestedValue]) => {
						const nestedSchema = entrySchema && typeof entrySchema === "object" && entrySchema !== null && "properties" in entrySchema && typeof entrySchema.properties === "object" && entrySchema.properties !== null ? entrySchema.properties[nestedKey] : void 0;
						const formatted = formatDefaultEntryValue(nestedValue, nestedSchema);
						return formatted === void 0 ? void 0 : `${JSON.stringify(nestedKey)}: ${formatted}`;
					}).filter((entry) => entry !== void 0).join(", ");
					return nestedEntries.length === 0 ? "{}" : `{ ${nestedEntries} }`;
				}
				if (isNumber(entryValue)) return `${entryValue} as const`;
				if (entryValue === null || entryValue === void 0 || isBoolean(entryValue)) return `${entryValue}`;
			};
			const properties = schema.properties && isObject(schema.properties) ? schema.properties : void 0;
			const entries = Object.entries(schema.default).map(([key, value]) => {
				const formatted = formatDefaultEntryValue(value, properties?.[key]);
				return formatted === void 0 ? void 0 : `${JSON.stringify(key)}: ${formatted}`;
			}).filter((entry) => entry !== void 0).join(", ");
			defaultValue = entries.length === 0 ? `{}` : `{ ${entries} }`;
			if (properties !== void 0 && Object.values(properties).some((p) => needsInlineDefault(p))) {
			...
			if (Array.isArray(schema.default) && (type === "array" || type === "tuple") && "prefixItems" in schema && schema.default.length > 0) {
				defaultVarName = defaultValue;
				defaultValue = void 0;
			}
		}
	...
	if (!skipSwitchStatement) switch (type) {
		...
		case "string":
			...
			if (schema.format === "date-time" || schema.format === "date-time-local") {
				const options = schema.format === "date-time-local" ? { local: true } : context.output.override.zod.dateTimeOptions;
	...
	if (!hasNonArrayEnum && isString(type) && minAndMaxTypes.has(type)) {
		const shouldUseExclusiveMin = exclusiveMin !== void 0;
		const shouldUseExclusiveMax = exclusiveMax !== void 0;
		if (shouldUseExclusiveMin && exclusiveMin !== void 0) {
			const safeExclusiveMin = assertSafeNumericConstraint(exclusiveMin, "exclusiveMinimum");
			consts.push(`export const ${name}ExclusiveMin${constsCounterValue} = ${safeExclusiveMin};`);
			functions.push(["gt", `${name}ExclusiveMin${constsCounterValue}`]);
		} else if (min !== void 0) {
			const safeMin = assertSafeNumericConstraint(min, "minimum");
			if (safeMin === 1) functions.push(["min", `${safeMin}`]);
			else {
				consts.push(`export const ${name}Min${constsCounterValue} = ${safeMin};`);
				functions.push(["min", `${name}Min${constsCounterValue}`]);
			}
		}
		if (shouldUseExclusiveMax && exclusiveMax !== void 0) {
			const safeExclusiveMax = assertSafeNumericConstraint(exclusiveMax, "exclusiveMaximum");
			consts.push(`export const ${name}ExclusiveMax${constsCounterValue} = ${safeExclusiveMax};`);
		...
		if (multipleOf !== void 0) {
			const safeMultipleOf = assertSafeNumericConstraint(multipleOf, "multipleOf");
			consts.push(`export const ${name}MultipleOf${constsCounterValue} = ${safeMultipleOf.toString()};`);
...
const buildMetaArgs = (args) => {
	const parts = [];
	if (args.id !== void 0) parts.push(`id: '${jsStringLiteralEscape(args.id)}'`);
	if (args.title !== void 0) parts.push(`title: '${jsStringLiteralEscape(args.title)}'`);
	if (args.description !== void 0) parts.push(`description: '${jsStringLiteralEscape(args.description)}'`);
...
${Object.entries(objectArgs).map(([key, schema]) => {
		...
		for (let index = 0; index < definition.functions.length; index++) {
			...
			if (fn === "fileOrString") {
				current = {
					expr: zodMiniCall("union", `[${zodMiniCall("instanceof", "Blob")}, ${zodMiniCall("string")}]`),
					kind: "union"
				};
				continue;
			}
...
${Object.entries(mergedProperties).map(([key, schema]) => {
	...
	const parseProperty = (property, fieldPath = []) => {
		...
		if (fn === "fileOrString") return "zod.instanceof(Blob).or(zod.string())";
		if (fn === "allOf") {
			const allOfArgs = args;
			if (strict) {
...
${Object.entries(objectArgs).map(([key, schema]) => {
		...
		if (fn === "tuple") return `zod.tuple([${args.map((x) => {
			...
			if (Array.isArray(x.consts)) appendConstsChunk(x.consts.join("\n"));
			return `${value.startsWith(".") ? "zod" : ""}${value}`;
		}).join(",\n")}])`;
		if (fn === "rest") {
			const restArgs = args;
			const restValue = restArgs.functions.map((prop) => parseProperty(prop, fieldPath)).join("");
			if (Array.isArray(restArgs.consts)) appendConstsChunk(restArgs.consts.join("\n"));
...
function tryResolveRefSchema($ref, context) {
	try {
		...
		logger.verbose(`Failed to resolve $ref "${$ref}": ${error instanceof Error ? error.message : String(error)}`);
		return;
	}
}
...
const parseBodyAndResponse = ({ data, context, name, strict, generate, isZodV4, parseType, useReusableSchemas }) => {
	...
	if (resolvedJsonSchema.items) {
		const rawMin = resolvedJsonSchema.minimum ?? resolvedJsonSchema.minLength ?? resolvedJsonSchema.minItems;
		const rawMax = resolvedJsonSchema.maximum ?? resolvedJsonSchema.maxLength ?? resolvedJsonSchema.maxItems;
...
const generateZodRoute = async ({ operationId, operationName, typeName, verb, override }, { pathRoute, context, output }) => {
	...
	const renderExport = (name, expression, arrayItem) => renderZodExport({
		name,
		expression,
		variant: zodVariant,
		brand: useBrandedTypes ? { isZodV4 } : void 0,
	...
	return {
		implementation: [
			...
			...inputQueryParams.zod ? [renderExport(queryParamsName, inputQueryParams.zod)] : [],
			...inputHeaders.consts ? [inputHeaders.consts] : [],
			...inputHeaders.zod ? [renderExport(headerName, inputHeaders.zod)] : [],
			...inputBody.consts ? [inputBody.consts] : [],
			...inputBody.zod ? [renderExport(bodyName, inputBody.zod, parsedBody.isArray ? { rules: parsedBody.rules } : void 0)] : [],
			...inputResponses.flatMap((inputResponse, index) => {
				...
				return [...inputResponse.consts ? [inputResponse.consts] : [], renderExport(operationResponse, inputResponse.zod, parsedResponses[index].isArray ? { rules: parsedResponses[index].rules } : void 0)];
			})
		].join("\n\n"),
		mutators: [
