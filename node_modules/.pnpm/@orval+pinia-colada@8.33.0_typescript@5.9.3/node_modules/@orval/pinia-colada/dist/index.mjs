...
import { OutputHttpClient, pascal } from "@orval/core";
	...
	exports: [
		{
			name: "useQuery",
			values: true,
			alias: "useColadaQuery"
		},
		{
			name: "useMutation",
			values: true,
			alias: "useColadaMutation"
		},
...
const builder = ({ output }) => () => {
	const isFetch = output.httpClient === OutputHttpClient.FETCH;
	const transport = isFetch ? fetch()() : axios()();
	const client = async (verb, options, outputClient, normalized) => {
		if (!isFetch && output.httpClient !== OutputHttpClient.AXIOS) throw new Error("pinia-colada supports the fetch and axios HTTP clients.");
		if (verb.mutator?.isHook) throw new Error("pinia-colada requires a plain function mutator, not a hook mutator.");
		...
		const generated = await transport.client(requestVerb, options, outputClient, normalized);
		const name = verb.operationName;
		const title = pascal(name);
		const data = `Awaited<ReturnType<typeof ${name}>>`;
		const names = /* @__PURE__ */ new Set([name, ...verb.props.map((prop) => prop.name)]);
		...
		const hasRequestOptions = verb.override.requestOptions !== false && (!verb.mutator || verb.mutator.hasSecondArg || isFetch);
		const requiredRequest = hasRequestOptions && output.optionsParamRequired;
		const optionalConfig = requiredRequest ? "" : "?";
		const requestOption = hasRequestOptions ? `request${optionalConfig}: Parameters<typeof ${name}>[${verb.props.length}];` : "";
		const args = verb.props.map((prop) => prop.name);
		const typeAt = (index) => `Parameters<typeof ${name}>[${index}]`;
		const plain = verb.props.map((prop, index) => `${prop.name}${prop.required ? "" : "?"}: ${typeAt(index)}`);
		const reactive = verb.props.map((prop, index) => `${prop.name}${prop.required ? "" : "?"}: MaybeRefOrGetter<${typeAt(index)}>`);
		const withRequiredConfig = (parameters) => requiredRequest ? parameters.map((parameter) => parameter.replace("?:", ":")) : parameters;
		...
		if (query) {
			...
			const requestArgs = [...args, ...hasRequestOptions ? [`{ ...${config}?.request, signal: ${signal} }`] : []];
			wrapper = `
...
export function use${title}${generic}(${[...withRequiredConfig(reactive), `${config}${optionalConfig}: MaybeRefOrGetter<${configType}>`].join(", ")}) {
  return useColadaQuery(() => ${getOptions}(${[...args.map((arg) => `toColadaValue(${arg})`), `toColadaValue(${config})`].join(", ")}));
}
`;
		} else {
			const varsType = `${title}MutationVariables`;
			const props = verb.props.map((prop, index) => `${prop.name}${prop.required ? "" : "?"}: ${typeAt(index)}`);
			const generic = "<TError = globalThis.Error, TContext extends Record<string, unknown> = Record<string, never>>";
			...
			const requestArgs = [...args.map((arg) => `${variables}.${arg}`), ...hasRequestOptions ? [`${config}${requiredRequest ? "." : "?."}request`] : []];
			wrapper = `
...
`;
	...
	return {
		...transport,
		client,
		dependencies: (...args) => [...transport.dependencies?.(...args) ?? [], ...dependencies]
	};
};
//#endregion
