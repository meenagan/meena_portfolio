...
*/
const getArrayResponseSchema = (imports, definition, toValueRef = (typeName) => typeName) => {
	if (definition === void 0) return void 0;
	const elementName = ARRAY_DEFINITION_PATTERN.exec(definition)?.[1];
	if (elementName === void 0 || isPrimitiveType(elementName)) return;
	if (!imports.some((imp) => imp.name === elementName)) return void 0;
	return {
		elementName,
...
*/
const getZodNamespaceImportSource = (output) => output.override.zod.variant === "mini" ? "zod/mini" : "zod";
/** The namespace import a composed `zod.array(...)` expression requires. */
const getZodNamespaceImport = (output) => ({
	name: "zod",
	values: true,
...
*/
const resolveValidatableSchema = (imports, value) => {
	if (isPrimitiveType(value)) return void 0;
	if (hasSchemaImport(imports, value)) return {
...
const generateHttpClientImplementation = ({ headers, queryParams, operationName, typeName, response, mutator, body, props, verb, override, formData, formUrlEncoded, paramsSerializer, paramsFilter, params }, { route: _route, context }) => {
	...
	const arraySchema = getArrayResponseSchema(response.imports, dataType, getSchemaValueRef);
	const isZodOutput = isZodSchemaOutput(context.output);
	const shouldValidateResponse = override.angular.runtimeValidation.enabled && isZodOutput && !isPrimitive && (hasSchema || !!arraySchema);
	const parsedDataType = shouldValidateResponse ? arraySchema?.outputTypeRef ?? getSchemaOutputTypeRef(dataType) : dataType;
	const getGeneratedResponseType = (value, contentType) => {
		if (override.angular.runtimeValidation.enabled && isZodOutput && !!contentType && (contentType.includes("json") || contentType.includes("+json"))) {
	...
	const schemaValueRef = shouldValidateResponse ? arraySchema?.schemaRef ?? getSchemaValueRef(dataType) : dataType;
  ...
  `;
	...
	const jsonSchema = jsonSuccessValues.length === 1 && override.angular.runtimeValidation.enabled && isZodOutput ? resolveValidatableSchema(response.imports, jsonSuccessValues[0]) : void 0;
	...
	if (hasMultipleContentTypes && !shouldValidateResponse && jsonSchema) jsonValidationPipe = emitResponseValidation({
...
const narrowsResponseEvents = ({ response, override }, output) => {
	...
	return override.requestOptions !== false && !hasMultipleContentTypes && override.angular.runtimeValidation.enabled && isZodSchemaOutput(output) && !isPrimitiveType(dataType) && (hasSchemaImport(response.imports, dataType) || !!getArrayResponseSchema(response.imports, dataType));
};
/**
...
const generateAngular = (verbOptions, options) => {
	...
	const normalizedVerbOptions = (() => {
		...
		const responseArraySchema = getArrayResponseSchema(result.response.imports, responseType);
		const schemaImportName = hasSchemaImport(result.response.imports, responseType) ? responseType : responseArraySchema?.elementName;
		if (!isPrimitiveResponse && schemaImportName !== void 0) result = {
			...result,
			response: {
				...result.response,
				imports: [...result.response.imports.map((imp) => imp.name === schemaImportName ? {
					...imp,
					values: true
				} : imp), { name: getSchemaOutputTypeRef(schemaImportName) }]
			}
		};
		...
		if ([...new Set(successTypes.map((t) => t.contentType).filter(Boolean))].length > 1) {
			...
			if (jsonSchemaNames.length === 1) {
				...
				const jsonImportName = hasSchemaImport(result.response.imports, jsonType) ? jsonType : getArrayResponseSchema(result.response.imports, jsonType)?.elementName;
				if (!jsonIsPrimitive && jsonImportName !== void 0) result = {
					...result,
					response: {
						...result.response,
						imports: [...result.response.imports.map((imp) => imp.name === jsonImportName ? {
							...imp,
							values: true
						} : imp), { name: getSchemaOutputTypeRef(jsonImportName) }]
					}
				};
			}
		}
	...
	return {
		...
		imports: [
			...
			...implementation.includes("zod.array(") ? [getZodNamespaceImport(options.context.output)] : [],
			...baseUrl ? [{
...
const applySignalRoute = (route, params, useNamedParams) => {
	...
	return mapTemplateExpressions(route, (expression) => {
		...
		if (useNamedParams) return defaultValue === void 0 ? "pathParams()." + param.name : "pathParams()?." + param.name + " ?? " + defaultValue;
		return defaultValue === void 0 ? param.name + "()" : param.name + "?.() ?? " + defaultValue;
	});
};
/**
...
const resolveParseSchema = (response, factory, output, responseTypeOverride) => {
	if (factory !== "httpResource") return void 0;
	const zodSchema = response.imports.find((imp) => imp.isZodSchema);
	if (zodSchema) return {
		importName: zodSchema.name,
		schemaRef: zodSchema.name
	};
	...
	if (response.imports.some((imp) => imp.name === responseType)) return {
		importName: responseType,
		schemaRef: responseType
	};
	const arraySchema = getArrayResponseSchema(response.imports, responseType);
	return arraySchema ? {
		importName: arraySchema.elementName,
		schemaRef: arraySchema.schemaRef
	} : void 0;
};
/** The import name to promote to a value import for a validated response. */
const getParseSchemaName = (response, factory, output, responseTypeOverride) => resolveParseSchema(response, factory, output, responseTypeOverride)?.importName;
...
*/
const needsZodNamespaceImport = (response, output) => response.types.success.some((successType) => resolveParseSchema(response, getHttpResourceFactory(response, successType.contentType, successType.value), output, successType.value)?.schemaRef.startsWith("zod."));
const getHttpResourceVerbImports = (verbOptions, output) => {
	...
	const parsedZodImportNames = isZodOutput ? getHttpResourceZodParsedImportNames(response, output) : /* @__PURE__ */ new Set();
	const parsedZodImports = responseImports.filter((imp) => parsedZodImportNames.has(imp.name));
	return [
		...isZodOutput && needsZodNamespaceImport(response, output) ? [getZodNamespaceImport(output)] : [],
...
const getParseExpression = (response, factory, output, operationName, responseTypeOverride) => {
	const resolved = resolveParseSchema(response, factory, output, responseTypeOverride);
	return resolved ? emitResponseValidation({
...
const buildHttpResourceFunction = (verbOption, route, output) => {
	...
	const dataArraySchema = getArrayResponseSchema(responseSchemaImports, dataType);
	const resourceName = `${operationName}Resource`;
	const parsedDataType = omitParse && output.override.angular.runtimeValidation.enabled && !isPrimitiveType(dataType) && (hasResponseSchemaImport || !!dataArraySchema) ? dataArraySchema?.outputTypeRef ?? getSchemaOutputTypeRef(dataType) : dataType;
	const successTypes = response.types.success;
...
const getHttpResourceGeneratedResponseType = (value, contentType, responseImports, output) => {
	if (isZodSchemaOutput(output) && output.override.angular.runtimeValidation.enabled && !!contentType && (contentType.includes("json") || contentType.includes("+json")) && !isPrimitiveType(value)) {
		if (responseImports.some((imp) => imp.name === value)) return getSchemaOutputTypeRef(value);
